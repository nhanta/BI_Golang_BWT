...
func GetApproximatePatternMatchingWithCheckPointsC (bwt string, sra [][] string, psa map[int] int,
	                                                ...
	                                                d int) map[string] [] map[int] map[int] int {
	...
	for g := 0; g < goroutines; g++ {
		go func(g int) {
			...
			for i := start; i < end; i++{
				...
				go func() {
					pos1 := GetSubApproximateStringWithCheckPointsC(bwt, alphabet, pattern1, sraName, d,
				...
				go func() {
					pos2 := GetSubApproximateStringWithCheckPointsC(bwt, alphabet, pattern2, sraName, d,
						initialTop, initialBottom,
						psa, countDict, firstOccurrence)
					rCh <- pos2
					w.Done()
...
func main(){
	...
	d := 1
	fmt.Println("Mismatch Threshold: ", d)
